...
##        shutil.copytree(os.path.abspath(self._packagePath), worDir)
    ...
    def simulate_translated(self):
        ...
        shutil.copytree(self._translateDir_, worDir, dirs_exist_ok = True)
        ...
        try:
            ...
            for c in dec:
                fil.write(c + ';\n')
            fil.write('resultFile=' + '"' + str(
                self._simulator_.get('resultFile')) + '"' + ';\n')
            ...
            if _platform == "linux" or _platform == "linux2":
                # linux
                fil.write('system("mv dsres.mat ' + self._simulator_.get(
                    'resultFile') + '.mat");\n')
                ...
                # Windows
                fil.write('system("ren dsres.mat ' + self._simulator_.get(
                    'resultFile') + '.mat");\n')
            ...
            # Run simulation
            fd = os.open(os.path.join(worDir, 'dsin.txt'), os.O_RDONLY)
            self._runSimulation(runScriptName,
                                self._simulator_.get('timeout'),
                                worDir)
            os.close(fd)
            self._check_simulation_errors(worDir)
            # TODO: edit copyResultFiles and deleteTemporaryDirectory
